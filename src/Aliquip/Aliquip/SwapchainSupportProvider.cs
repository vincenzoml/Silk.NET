...
namespace Aliquip
{
    internal sealed class SwapchainSupportProvider : ISwapchainSupportProvider
    {
        ...
        public unsafe SwapchainSupportDetails QuerySwapchainSupport(PhysicalDevice device)
        {
            ...
            _khrSurface.GetPhysicalDeviceSurfaceCapabilities(device, _surfaceProvider.Surface, out details.Capabilities).ThrowCode();

            uint count = 0;
            _khrSurface.GetPhysicalDeviceSurfaceFormats(device, _surfaceProvider.Surface, ref count, null).ThrowCode();
            var formats = new SurfaceFormatKHR[(int) count];
            fixed (SurfaceFormatKHR* pformats = formats)
                _khrSurface.GetPhysicalDeviceSurfaceFormats(device, _surfaceProvider.Surface, ref count, pformats).ThrowCode();
            details.Formats = formats;

            count = 0;
            _khrSurface.GetPhysicalDeviceSurfacePresentModes(device, _surfaceProvider.Surface, ref count, null).ThrowCode();
            var modes = new PresentModeKHR[(int) count];
            fixed (PresentModeKHR* pmodes = modes)
                _khrSurface.GetPhysicalDeviceSurfacePresentModes(device, _surfaceProvider.Surface, ref count, pmodes).ThrowCode();
