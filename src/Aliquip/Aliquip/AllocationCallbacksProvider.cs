...
namespace Aliquip
{
    internal sealed class AllocationCallbacksProvider : IAllocationCallbacksProvider, IDisposable
    {
            ...
            SystemAllocationScope allocationScope)
        {
            ...
            if (isAllocation)
            {
                @this._logger.LogTrace
                    ("Reallocation as Allocation: {size} {alignment} {scope}", size, alignment, allocationScope);
                return Allocate(size, alignment);
            }

            if (isFree)
            {
                @this._logger.LogTrace("Reallocation as Free: {alignment} {scope}", alignment, allocationScope);
                Free(pOriginal);
                return null;
            }

            @this._logger.LogTrace("Reallocation: {size} {alignment} {scope}", size, alignment, allocationScope);
            var oldAlignment = *((UIntPtr*) pOriginal - 2);
            if (alignment != oldAlignment)
                @this._logger.LogWarning("Old alignment was unequal to new alignment, the spec forbids this. ({old}, {new})", oldAlignment, alignment);
            ...
            (void* pUserData, nuint size, InternalAllocationType allocationType, SystemAllocationScope allocationScope)
        {
            ...
            @this._logger.LogTrace("Internal Allocation: {type} {scope}", allocationType, allocationScope);
            ...
            (void* pUserData, nuint size, InternalAllocationType allocationType, SystemAllocationScope allocationScope)
        {
            ...
            @this._logger.LogTrace("Internal Free: {type} {scope}", allocationType, allocationScope);
